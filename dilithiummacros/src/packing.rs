...
pub(crate) fn mode_packing_internal(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let mode = syn::parse_macro_input!(input as Mode);
    let mode_name = mode.mode_name;

    let tokens = quote! {
        pub mod packing {
            use crate::{#mode_name::params::*, #mode_name::poly::*, #mode_name::polyvec::*, #mode_name::api::SignError};
